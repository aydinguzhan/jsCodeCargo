...
import path$1, { normalize } from "node:path";
...
function disposeSession(terminalId, session) {
...
function ownsSession(sender, terminalId) {
  const session = sessions.get(terminalId);
  return (session == null ? void 0 : session.owner.id) === sender.id ? session : void 0;
}
function createTerminal(event, { cwd, cols, rows }) {
  const shell = shellCommand();
  return startTerminal(event, shell.file, shell.args, { cwd, cols, rows });
}
function startTerminal(event, file, args, { cwd, cols, rows }) {
...
function runFileTerminal(event, options) {
  const command = commandForFile(options.filePath);
  if (!command) {
    return {
      error: `No runner is configured for ${path$1.extname(options.filePath) || "this file type"}.`
    };
  }
  try {
...
function writeTerminal(event, terminalId, data) {
  var _a2;
  (_a2 = ownsSession(event.sender, terminalId)) == null ? void 0 : _a2.process.write(data);
}
function resizeTerminal(event, terminalId, cols, rows) {
  var _a2;
  (_a2 = ownsSession(event.sender, terminalId)) == null ? void 0 : _a2.process.resize(
    Math.max(2, cols),
    Math.max(1, rows)
  );
}
...
function closeTerminalsFor(owner) {
...
const ignoredDirectoryNames = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "dist-electron"]);
...
async function findWorkspaceFiles(rootPath, query) {
  ...
  async function visit(directoryPath) {
    ...
    for (const entry of entries) {
      ...
      const entryPath = path$1.join(directoryPath, entry.name);
      ...
      const relativePath = path$1.relative(rootPath, entryPath);
      if (!normalizedQuery || entry.name.toLocaleLowerCase().includes(normalizedQuery) || relativePath.toLocaleLowerCase().includes(normalizedQuery)) {
...
async function searchWorkspaceText(rootPath, query) {
  const normalizedQuery = query.trim().toLocaleLowerCase();
  const matches = [];
  ...
  async function visit(directoryPath) {
    ...
    for (const entry of entries) {
      ...
      try {
        const info = await fs$1.stat(entryPath);
        if (info.size > 1024 * 1024) continue;
        const content = await fs$1.readFile(entryPath, "utf-8");
...
async function createNewFile(_2, name, extension, content) {
  try {
...
async function getGitOverview(cwd) {
...
async function stageFile(cwd, filePath) {
  try {
    await gitFor(cwd).add(["--", filePath]);
    return { success: true };
  } catch (error) {
    return { success: false, error: errorMessage(error) };
  }
}
async function unstageFile(cwd, filePath) {
  try {
    await gitFor(cwd).reset(["HEAD", "--", filePath]);
    return { success: true };
  } catch (error) {
    return { success: false, error: errorMessage(error) };
  }
...
async function createCommit(cwd, message) {
  try {
    await gitFor(cwd).commit(message);
    return { success: true };
  } catch (error) {
    return { success: false, error: errorMessage(error) };
  }
...
const settingsFilePath = () => path$1.join(app.getPath("userData"), "jscargocode-settings.json");
async function readSavedTheme() {
  try {
    const settings = JSON.parse(await fs$1.readFile(settingsFilePath(), "utf-8"));
...
async function saveTheme(theme) {
  await fs$1.mkdir(app.getPath("userData"), { recursive: true });
  await fs$1.writeFile(settingsFilePath(), JSON.stringify({ theme }), "utf-8");
}
function isPathInWorkspace(workspaceRoot, requestedPath) {
  const relativePath = path$1.relative(path$1.resolve(workspaceRoot), path$1.resolve(requestedPath));
  return relativePath === "" || !relativePath.startsWith(`..${path$1.sep}`) && relativePath !== ".." && !path$1.isAbsolute(relativePath);
}
function isWorkspaceRelativeFile(workspaceRoot, filePath) {
  return !path$1.isAbsolute(filePath) && isPathInWorkspace(workspaceRoot, path$1.resolve(workspaceRoot, filePath));
}
function createWindow() {
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    newWindow.loadFile(path$1.join(RENDERER_DIST, "index.html"));
  }
  return newWindow;
}
function registerIpcHandlers() {
  ...
  ipcMain.on(
    "terminal:write",
    (event, { terminalId, data }) => writeTerminal(event, terminalId, data)
  );
  ipcMain.on(
    "terminal:resize",
    (event, { terminalId, cols, rows }) => resizeTerminal(event, terminalId, cols, rows)
  );
  ...
  ipcMain.handle("settings:theme:set", (_2, theme) => saveTheme(theme));
  ipcMain.handle("create:file", createNewFile);
  ipcMain.handle(
    "write:file",
    (_2, filePath, content) => writeFile(filePath, content)
  );
  ...
  ipcMain.handle("workspace:restore-root", (event, rootPath) => {
    if (!rootPath) {
      return false;
    }
  ...
  ipcMain.handle("workspace:read-directory", (event, directoryPath) => {
  ...
  ipcMain.handle("workspace:find-files", (event, rootPath, query) => {
    const workspaceRoot = workspaceRoots.get(event.sender.id);
    if (!workspaceRoot || path$1.resolve(rootPath) !== path$1.resolve(workspaceRoot)) {
      return [];
    }
    return findWorkspaceFiles(workspaceRoot, query);
  });
  ipcMain.handle("workspace:search-text", (event, rootPath, query) => {
    const workspaceRoot = workspaceRoots.get(event.sender.id);
    if (!workspaceRoot || path$1.resolve(rootPath) !== path$1.resolve(workspaceRoot)) {
      return [];
    }
  ...
  ipcMain.handle("git:stage-file", (event, filePath) => {
    const workspaceRoot = workspaceRoots.get(event.sender.id);
  ...
  ipcMain.handle("git:unstage-file", (event, filePath) => {
    const workspaceRoot = workspaceRoots.get(event.sender.id);
    if (!workspaceRoot || !isWorkspaceRelativeFile(workspaceRoot, filePath)) return { success: false, error: "Invalid workspace file" };
  ...
  ipcMain.handle("file:read", (_2, filePath) => readFile(filePath));
}
